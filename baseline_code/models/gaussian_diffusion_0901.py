			...
			with th.no_grad():
				if reverse:         # reverse == encoding
					...
					for ith in range(32):
						...
						decoding_score_mean_images.save('/chenxue/experiment_data/loop_progressive_0831/decoding/score_mean_' + str(step).zfill(4) + '_' + str(ith).zfill(4) + '.jpg')
				...
				if step >= 0:
					...
					# print(patient)
					# print(slice)
					out_path = os.path.join(logger.get_dir(), f"/chenxue/experiment_data/sample_0831/samples_{str(step).zfill(4)}_{str(currt_len).zfill(4)}.npz")
					out_image = os.path.join(logger.get_dir(), f"/chenxue/experiment_data/sample_0831/samples_{str(step).zfill(4)}_{str(currt_len).zfill(4)}.png")
					print('out_npz_path:{}'.format(out_path))
					print('out_tif_path:{}'.format(out_image))
					# img.save(out_image, compression='raw')
					# ^^^^^^^^^^^^^^^^^^^^^^^^^^
