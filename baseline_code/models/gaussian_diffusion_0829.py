...
class GaussianDiffusion:
	...
	def ddim_sample_loop_progressive(
		...
		print('shape:{}'.format(len(shape)))                # 4
		...
		for step, i in enumerate(indices):  # 遍历，将indices送入ddim_sample中，
			...
			with th.no_grad():
				...
				if (step + 1) % 5 == 0:
					...
					print('type(output_data):{}'.format(type(output_sample)))
					for ith in range(32):
						...
						print('output_sample images.size:{}'.format(images_tensor.size()))  # torch.size([64, 320])
						images_numpy = images_tensor.cpu().numpy()
						images = Image.fromarray(np.uint8(images_numpy))
						# save_image(images, 'input_png')
