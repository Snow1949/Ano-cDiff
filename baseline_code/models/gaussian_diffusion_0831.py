...
class GaussianDiffusion:
	...
	def ddim_sample_loop_progressive(
		...
		print('indices(0 -> L*T): {}'.format(indices))
		...
		image = []
		for step, i in enumerate(indices):  # 遍历，将indices送入ddim_sample中，
			# ^^^^^^^^^^^^^^^^^^^^^^^^^^
			# step=0
			# split:test, patient_id:2, slice_id:43
			# ^^^^^^^^^^^^^^^^^^^^^^^^^^
			# ^^^^^^^^^^^^^^^^^^^^^^^^^^
			# split:test, patient_id:2, slice_id:44
			# ^^^^^^^^^^^^^^^^^^^^^^^^^^
			# step=1
			# ^^^^^^^^^^^^^^^^^^^^^^^^^^
			# split:test, patient_id:3, slice_id:17
			# ^^^^^^^^^^^^^^^^^^^^^^^^^^
			print('step={},i={}'.format(step, i))       # step=1...74
			t = th.tensor([i] * shape[0], device=device)
			print("t:{}".format(t))
			...
			with th.no_grad():
				if reverse:         # reverse == encoding
					...
					# 此处每次先输出104个：
					# ^^^^^^^^^^^^^^^^^^^^^^^^^^
					# split:test, patient_id:2, slice_id:45
					# ^^^^^^^^^^^^^^^^^^^^^^^^^^
					print('encoding')
