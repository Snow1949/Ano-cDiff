...
class GaussianDiffusion:
	...
	def ddim_sample_loop_progressive(
		...
		"""
        Use DDIM ######print finsihed######to sample from the model and yield intermediate samples from
		...
		for step, i in enumerate(indices):  # 遍历，将indices送入ddim_sample中，
			...
			with th.no_grad():
				...
				if (step + 1) % 2 == 0:
					...
					# print(len(image))
					currt_len = len(image)
					print('current(step={}) image:'.format(step))
					# print(image.shape)
					# print(image)
					print('++++++')
					...
					out_path = os.path.join(logger.get_dir(), f"/chenxue/experiment_data/anomaly_diffusion_healthy_only_train_brats2021_64x64/samples_label_{str(reverse)}_{str(currt_len).zfill(4)}_{str(step).zfill(4)}.npz")
					out_image = os.path.join(logger.get_dir(), f"/chenxue/experiment_data/anomaly_diffusion_healthy_only_train_brats2021_64x64/samples_label_{str(reverse)}_{str(currt_len).zfill(4)}_{str(step).zfill(4)}.png")
					print('out_npz_path:{}'.format(out_path))
					print('out_tif_path:{}'.format(out_image))
					# img.save(out_image, compression='raw')
					datas = img.getdata()
					...
					for ith in range(32):
						...
						images = Image.fromarray(np.uint8(images_numpy))
						# save_image(images, 'input_png')
						images.save('result_img/ddim_sample_loop_progressive/step'+ str(step) + '_' + str(ith) + '.jpg')
					print('######################')
				yield out
				img = out["sample"]
