...
from baseline_code.utils.fp16_util import convert_module_to_f16, convert_module_to_f32
from baseline_code.models.nn import avg_pool_nd, conv_nd, linear, normalization, timestep_embedding, zero_module
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    # x即t时刻x[t]，emb可能是timestep+condition
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if self.num_classes is not None:    # 如果是条件生成，则还有label_emb项
            self.label_emb = nn.Embedding(self.num_classes + 1, model_channels, padding_idx = -1)

        ch = input_ch = int(channel_mult[0] * model_channels)

        # 把所有Module放到ModuleList这个容器中，实例化对象一般为列表
        self.input_blocks = nn.ModuleList(
            # 第一层input_blocks就是个卷积
            # dims为几就是几维卷积，conv_nd层默认不改变空间结构
            ...
            for _ in range(num_res_blocks):
                layers = [
                    ResBlock(
                        ...
                        time_embed_dim,
                        dropout,
                        out_channels=int(mult * model_channels),    # 输出通道数，在扩大
                        dims=dims,
                ...
                # ds:下采样比例
                if ds in attention_resolutions:     # 如果ds在attention_resolutions列表里面
                    layers.append(                  # 插入AttentionBlock块
                        AttentionBlock(
                            ch,
                            use_checkpoint=use_checkpoint,
                            num_heads=num_heads,
