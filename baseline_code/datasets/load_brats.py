...
class PatientDataset(torch.utils.data.Dataset):
	def __init__(self, patient_dir: Path, process_fun=None, id=None, skip_condition=None, cache=True):
		...
		self.id = id
		self.len = len(self.slice_paths)  # len(slice-"patient_BraTS2021_00723")=135 当前受试者(病人)文件夹下所有切片数量
		self.idx_map = {x: x for x in range(self.len)}

		# print("self.patient_dir:{}".format(self.patient_dir))
		# print("self.skip_condition:{}".format(self.skip_condition))
				...
				for idx in range(self.len):  # self.len = 135
					...
					with np.load(self.slice_paths[idx]) as data:
						...
						if not skip_condition(item):
							...
							print('skip slice___patent:{}, idx:{}'.format(self.id, idx))
				self.len = len(valid_indices)
				print("patient:{}_slice_len:{}".format(int(self.id) ,int(self.len)))
...
class BrainDataset(torch.utils.data.Dataset):
	...
	def __init__(self, datapath: Path, dataset="brats2021_64x64", split="val",
		...
		print('*************')
		print("test patient_dirs:{}".format(len(patient_dirs)))      # patient_dirs:939(到此是训练集中所有受试者文件夹)
		print(patient_dirs)
		print('*************')
		if split == "train":
		...
		# 训练集中被确定为健康/患病受试者的文件夹不重叠
		print("self.n_tumour_patients:{}".format(self.n_tumour_patients))  # self.n_tumour_patients:939/250
		print("self.n_healthy_patients:{}".format(self.n_healthy_patients))  # self.n_healthy_patients:0/0
		...
		for i in range(self.n_tumour_patients):
			print("id:{}, len:{}".format(self.patient_datasets[i].id, self.patient_datasets[i].len))
	...
	def __getitem__(self, idx):
		...
		# cord是一个Batch*3*Height*Width张量，它提供了一个范围为(- 1, 1)的浮点数，用于对象的粗略“位置”
		# 3个轴中3D大脑中的  # 像素。可以忽略。
