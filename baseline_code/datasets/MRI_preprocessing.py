...
def process_patient(path, target_path):
	...
	for slice_idx in range(fs_dim2, ls_dim2):
		low_res_x = F.interpolate(volume[:, :, :, :, slice_idx], mode="bilinear", size=(64, 64))
		low_res_y = F.interpolate(labels[:, :, :, :, slice_idx], mode="bilinear", size=(64, 64))
...
def preprocess(datapath: Path):
	# datapath: Path("/chenxue/dataset/brain/BraTS2021_Training_Data/")
	...
	# # for split in ["train", "val", "test"]:
	#
	# splits_path = "/chenxue/Diff-SCM-main/diff_scm/datasets/data/brats2021_preprocessed/data_splits/test/scans.csv"
	# paths = [datapath / x.strip() for x in open(splits_path).readlines()]
	#
	...
	# 	target_path = Path("/chenxue/Diff-SCM-main/diff_scm/datasets/data/brats2021_preprocessed/npy_test")
	...
	for split in ["train", "val", "test"]:
		...
		for source_path in tqdm(paths):
			target_path = Path(__file__).parent.parent / "data" / "brats2021_64x64" / f"npy_{split}"
			process_patient(source_path, target_path)
...
# cd /chenxue/Diff-SCM-main/diff_scm/datasets && python -u MRI_preprocessing.py
# cd /chenxue/Diff-SCM-main/diff_scm/datasets && python -u MRI_preprocessing.py